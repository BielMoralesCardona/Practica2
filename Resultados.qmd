...
data_opt <- data %>%
  mutate(diagnostico = fct_collapse(diagnostico,
    ...
    # "Cordes vocals" = c("Càncer cordes vocals", "disfonia", "Nódulo cuerdas vocales", "Nòduls"),
...
Hemos conservado los grupos preestablecidos y creado dos más con los individuos que han ecrito sus enfermedades en la opción "Otro: ______". Han sido Altres que reune enfermedades que no se pueden asociar con ningún grupo anterior, y Cordes_vocals que reune enfermedades que afectan directamente en las cuerdas vocales como cáncer o nódulos en esa zona.
...
Comprovemos la normalidad de los datos para intentar aplicar un ANOVA paramétrico:



```{r}
#modelo_anova <- aov(VHI ~ diagnostico, data = data_opt)
...
Notemos que podemos asumir que todas siguen una distribución normal excepot los grupos "Sense patologia" y "Pulmonar". De este modo, no podemos continuar la prueba con un test paramétrico, tenemos que comprovar las medias con un test de ANOVA no paramétrico.

```{r}
friedman.test(VHI ~ diagnostico, data = data_opt)
```
no hay dif???
